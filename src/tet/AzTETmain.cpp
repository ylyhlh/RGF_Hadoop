...
void AzTETmain::train(const char *argv[], int argc)
{
  ...
  bool success = resetParam_train(argv, argc, for_train_test);//set up the parameters
  if (!success) return;
  ...
  /*---  read training data  ---*/
  AzDvect v_tr_y, v_fixed_dw; //output and weight using dense vector class
  AzSmat m_tr_x; // input using sparse matrix
...
bool AzTETmain::resetParam_train(const char *argv[], int argc,
                                 bool for_train_test)
{
  // We have more than 3 parameters for cluster now!
