...
AzTETrainer_Ret AzRgforest::proceed_until()
{
  ...
  for ( ; ; ) {
    ...
    if (opt_timer.ringing(false, l_num)) {
      ...
      cerr << "preoptAllreduce_time: " << Hadoop::all_reduce_watch.get_total() << " data(bytes): " << Hadoop::all_reduce_data_amount << " count: " << Hadoop::all_reduce_counter << endl; 
      optimize_resetTarget();
      cerr << "postoptAllreduce_time: " << Hadoop::all_reduce_watch.get_total() << " data(bytes): " << Hadoop::all_reduce_data_amount << " count: " << Hadoop::all_reduce_counter << endl; 
      show_tree_info(); 
    }
