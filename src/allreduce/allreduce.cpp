...
void all_reduce_init(const string master_location, const size_t unique_id, const size_t total, const size_t node, node_socks& socks)
{
  ...
  /// Get # of children. Listen to children if it is needed.
  uint16_t kid_count;
  ...
  /// Report ports for listening to children to master
  if(send(master_sock, (const char*)&netport, sizeof(netport), 0) < (int)sizeof(netport))
    cerr << "write failed!" << endl;

  /// Get parent address, root's is -1
  ...
  /*
   * Everyone except the root try to connet its parent, blcoking here.
   * Only root accept its children. Then children turn to accept their children.
   */
...
void addbufs(double* buf1, const double* buf2, const int n) {
  for(int i = 0;i < n;i++)
    //     {
...
void pass_up(char* buffer, int left_read_pos, int right_read_pos, int& parent_sent_pos, socket_t parent_sock, int n) {
  int my_bufsize = min(buf_size, ((int)(floor(left_read_pos/((double)sizeof(double)))*sizeof(double)) - parent_sent_pos));
  my_bufsize = min(my_bufsize, ((int)(floor(right_read_pos/((double)sizeof(double)))*sizeof(double)) - parent_sent_pos));
