  ...
  my $inp_fn = $ARGV[$argx++];
  my $rest_params = "";
  # perl call_exe.pl master unique_id total node_id
  if ($arg_num > 3) {
    my $master = $ARGV[$argx++];
    my $unique_id = $ARGV[$argx++];
    my $total = $ARGV[$argx++];
    my $node_id = $ARGV[$argx++];
    $rest_params = "--master $master --unique_id $unique_id --total $total --node_id $node_id";
  }
  ...
  if ($repeat_num == 0) {
    my $cmd = "$exe $action $config $rest_params";
    print "$cmd\n";
    system($cmd);
  }
  else {
    ...
    for ($rx = 0; $rx < $repeat_num; ++$rx) {
      ...
      my $cmd = "$exe $action $my_config $rest_params";
      print "$cmd\n";
      system($cmd);
      my $ret = $?;
      if ($ret != 0) {
